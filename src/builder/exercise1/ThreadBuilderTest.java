...
public class ThreadBuilderTest {
    ...
    private void testDaemonChild(boolean parentDaemon,
        ...
        ThreadBuilder parentBuilder = new ThreadBuilder(
            () -> {
            ...
            "parent")
            .daemon(parentDaemon);
        Thread parent = parentBuilder.build();
        parent.start();
        parent.join();
        assertEquals("ThreadBuilderTest.testDaemonChild " +
                "parentDaemon = " + parentDaemon + ", " +
                "childDaemonOverride = " + childDaemonOverride + ", " +
                "childDaemonExpected = " + childDaemonExpected,
            childDaemonExpected, childIsDaemon.get());
    }
    ...
    public void testUncaughtExceptionHandler() throws InterruptedException {
        ...
        test.join();
        assertTrue(ran.get());
        assertTrue("Gingerbread Man not caught", caught.get());
    }
}
